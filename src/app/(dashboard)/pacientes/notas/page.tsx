...
export default async function NotasPage() {
  ...
  const [{ data: pacientes }, { data: notas }] = await Promise.all([
    supabase.from('pacientes').select('*').order('apellido', { ascending: true }),
    supabase.from('paciente_notas').select('*').order('created_at', { ascending: false }),
  ])
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 w-full">
      <div className="flex items-center gap-3 mb-6 anim-fade-up">
        ...
        <span className="text-sm font-medium" style={{ color: 'var(--foreground)' }}>Notas</span>
      </div>
      ...
      <NotasClient
        ...
        initialNotas={(notas ?? []) as PacienteNota[]}
      />
    </div>
  )
}
